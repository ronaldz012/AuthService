...
using module.Auth.UseCases.branches;
using Npgsql;
    ...
    IConfiguration configuration) // Usado para "saltar" al nuevo esquema
{
    public async Task<Result<Guid>> CreateTenantAsync(CreateTenantDto dto)
    {
        ...
        if (await authContext.Tenants.AnyAsync(t => t.DisplayName == dto.DisplayName))
        ...
        authContext.Tenants.Add(newTenant);

        using var sharedTransaction = await authContext.Database.BeginTransactionAsync();
        try
        {
            ...
            var branchResult = await branchService.CreateBranch(new CreateBranchRequest
            {
                Name = dto.BranchName,
