...
namespace System.Api.Controllers.Auth
{
    ...
    public class UserController(UserUserCases userUserCases, ICurrentUser currentUser) : ControllerBase
    {

        [HttpGet]
        public async Task<IActionResult> GetUsers([FromQuery]UserQueryDto request)
        {
            return await userUserCases.GetAllUsers.Execute(currentUser.ToActorContext(), request).ToValueOrProblemDetails();
        }

        [HttpPost]
        public async Task<IActionResult> CreateUser([FromBody] CreateUserRequest request)
        {
            return await userUserCases.CreateUser.Execute(currentUser.ToActorContext(), request).ToValueOrProblemDetails();
        }

        [HttpPost("tenant-admin")]
        public async Task<IActionResult> CreateTenantAdmin([FromBody] CreateTenantAdminRequest request)
        {
            return await userUserCases.CreateTenantAdmin.Execute(currentUser.ToActorContext(), request).ToValueOrProblemDetails();
        }

        [HttpPatch("{id:guid}/status")]
        public async Task<IActionResult> UpdateUserStatus([FromRoute] Guid id)
        {
            return await userUserCases.UpdateUserStatus.Execute(currentUser.ToActorContext(), id).ToValueOrProblemDetails();
        }

        [HttpPut("{id:guid}")]
        public async Task<IActionResult> UpdateUser([FromRoute] Guid id, [FromBody] UpdateUserRequest request)
        {
            return await userUserCases.UpdateUser.Execute(currentUser.ToActorContext(), id, request).ToValueOrProblemDetails();
        }
        ...
        {
            return await userUserCases.ToggleUserType.Execute(currentUser.ToActorContext(), id).ToValueOrProblemDetails();
        }
    }
}
